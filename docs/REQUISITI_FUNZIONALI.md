...
- **Undo/Redo arredi camera**: in `RoomDetailDialog` le modifiche di `pos_x`/`pos_y` (drag o "Posiziona") e di `quantity` sono ora reversibili tramite bottoni dedicati nell'header del dialogo e scorciatoie tastiera Ctrl/Cmd+Z e Ctrl/Cmd+Shift+Z. Stack locale per camera (max 50 voci), resistente al realtime: il ripristino emette una UPDATE che sovrascrive eventuali valori arrivati via subscription.
- **Data Explorer (super admin)**: nuova rotta `/app/data-explorer` con CRUD universale su 80+ tabelle pubbliche raggruppate per dominio (Strutture & spazi, Asset, Ticket & SLA, Fornitori, Manutenzione, Inventario, Economato, Comunicazioni, Permessi, Workflow, Report, Abbonamento, Backup, Sistema, Audit). Include ricerca per tabella, paginazione (50/pag), filtro in pagina, export CSV/JSON, stampa lista e stampa scheda singola, editor di riga generato dalle colonne con parsing automatico di JSON/booleani/numeri/null. Le tabelle di audit e log sono in sola lettura. L'accesso è ristretto al ruolo `super_admin` ed è governato dalle RLS esistenti.
