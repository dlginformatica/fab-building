...
- **2026-06-20 — Fase 21.2 · Backup cloud, point-in-time, audit, reset org, schemi import versionati**
  - Nuovo bucket privato `org-backups` (path `<org_id>/<timestamp>__<kind>.json`) con policy RLS su `storage.objects` che limita read/write/delete al super_admin o all'owner dell'organizzazione.
  - Tabelle: `backup_runs(org_id, actor_id, kind[manual|scheduled|pre_restore|pre_reset], format, storage_bucket, storage_path, size_bytes, schema_version, tables_count, rows_count, snapshot_taken_at, status, details)`, `backup_schedules(org_id UNIQUE, enabled, frequency[daily|weekly|monthly], hour_utc, weekday, day_of_month, retention_count, format, last_run_at, next_run_at)`, `restore_runs(org_id, actor_id, source_backup_id→backup_runs, source_filename, mode[merge|replace|point_in_time], pit_target, pit_resolved_to, status, rows_inserted, errors_count, error_message, details)`, `import_mappings(org_id, target_table, name, schema_version, delimiter, mapping jsonb, fields_snapshot jsonb, notes)`.
  - RPC: `backup_record(...)`, `restore_record(...)`, `backup_nearest_to(_org,_target)` (SELECT 1 backup_runs con snapshot_taken_at ≤ target), `super_admin_reset_org(_org,_confirm,_note)` — verifica `has_role super_admin`, valida che `_confirm = organizations.name`, raccoglie i path storage, esegue DELETE a cascata su tutte le tabelle org-scoped (mantenendo `organizations`, `org_memberships` owner, `org_subscriptions`), inserisce `permission_audit` con `entity='organization'` e `action='reset_org'`; ritorna `jsonb { structures_deleted, storage_paths }` per consentire al client la rimozione degli oggetti storage.
  - Helper `src/lib/backup.ts` esteso: `uploadSnapshotAndRecord`, `downloadSnapshotFromStorage`, `signedBackupUrl`, `findNearestBackup`, `recordRestore`, `deleteBackup`, `buildMappingFile`/`parseMappingFile`. Costanti `BACKUP_BUCKET='org-backups'`, `SNAPSHOT_SCHEMA_VERSION=2`, `IMPORT_MAPPING_SCHEMA_VERSION=1`.
  - UI: `BackupPanel` ora ha un bottone "Backup nel cloud", la modalità restore **point_in_time** (datetime-local + conferma nome org → ricerca snapshot più vicino → pre_restore backup → replace + audit), e una tabella "Storico backup nel cloud" con download URL firmato (10 min), restore one-click e delete.
  - Nuova pagina `/app/backup-schedules`: configurazione pianificazione admin (frequenza, ora UTC, weekday/day-of-month, retention), bottone "Esegui adesso" che fa snapshot + upload + retention prune + aggiornamento `last_run_at`/`next_run_at`. Per il super_admin un riquadro "Pianificazioni scadute" mostra tutte le org con `next_run_at <= now()`.
  - Nuova pagina `/app/backup-audit` (admin & super_admin) con filtri data (`from`/`to`) e organizzazione, esport CSV (backup separato da restore) e PDF (finestra di stampa). Per gli admin org la query è auto-filtrata su `sub.orgId`.
  - Nuova pagina `/app/super-admin/reset-org` (gated `has_role super_admin`): selezione org, conferma nome, motivazione, checkbox "backup pre-reset" → chiama `super_admin_reset_org`, pulisce gli oggetti storage residui e invalida le query React Query.
  - `ImportWizard` ora permette di **Esportare il mapping** corrente come file `.mapping.json` con `schema_version`, `target_table`, `mapping` e snapshot dei campi attesi; e di **Caricare un mapping** esistente per riapplicarlo. Cambi di formato CSV nel tempo gestiti senza rifare la mappatura.
  - `/app/super-admin/subscription-audit` ora supporta filtri data + org ed export CSV/PDF (`src/lib/csv-export.ts` con `exportRowsAsCsv` via XLSX `sheet_to_csv` e `printableHtmlAsPdf` che apre una finestra stampabile).
  - Sidebar: nel gruppo Abbonamento aggiunte voci "Pianificazione backup", "Audit backup & restore", "Reset organizzazione (super admin)".
...
- **2026-06-20 — Fase 20.1 · Job sincronizzazione abbonamenti**
  ...
  - Funzione `public.super_admin_force_subscription(_org, _tier, _status, _extend_days, _note)`: consente al super_admin (e solo a lui) di forzare tier/stato, estendere trial o periodo attivo di N giorni, con tracciamento in `permission_audit` (entity=`org_subscription`, action=`force_override`).
  - UI `/app/super-admin/plans`: pulsanti "+30gg attivo", "+1 anno", "+30gg trial", "Blocca", cambio tier passano tutti dalla nuova RPC. Aggiunto pulsante "Sincronizza ora" per forzare il sync senza attendere l'orario.
  - Banner `SubscriptionBanner` e gate `useCanWrite()` invariati: leggono direttamente lo `status` aggiornato dal job.
